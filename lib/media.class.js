...
class MediaClass {
  ...
  async init() {
    ...
    this.ws.sendData('media/answer', this.answer);
    ...
    this.pairs.forEach((pair) => {
      ...
      this.ws.sendData('log/append', {
        ...
        message: pair.localCandidate,
      });
      console.log(`MEDIA pair remote ${pair.remoteCandidate}`);
      this.ws.sendData('log/append', {
        type: 'remote candidate',
        message: pair.remoteCandidate,
      });
    });
  }
  ...
  async createRecord() {
    ...
    const fileRecordUrl = `file://${config.get('filesFullPath')}`;
    ...
    if (this.client) {
      ...
      this.fileRecordEndpoint = await this.pipeline.create('RecorderEndpoint', {
        uri: `${fileRecordUrl}/${strTime}.mp4`,
      });
      await this.webRtcEndpoint.connect(this.recordEndpoint);
      await this.webRtcEndpoint.connect(this.fileRecordEndpoint);
      console.log(`MEDIA record stream here "${await this.recordEndpoint.getUri()}"`);
      console.log(`MEDIA record file here "${await this.recordEndpoint.getUri()}"`);

      this.recordEndpoint.record();
      this.fileRecordEndpoint.record();
      this.ws.sendData('media/recordStarted', await this.recordEndpoint.getUri());
      this.ws.sendData('log/append', {
        type: 'record',
        message: `in stream ${await this.recordEndpoint.getUri()}`,
      });
      this.ws.sendData('log/append', {
        type: 'record',
        message: `in local ${await this.fileRecordEndpoint.getUri()}`,
      });
    }
  }
  ...
  static assignWebSocket() {
    socket
      ...
      .setHandler('media/record-start', (data, ws) => {
        /** @type {MediaClass} */
        const media = ws.media;
